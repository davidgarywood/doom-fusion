...
int infinite_ammo; // Hakx 7/19/1999
...
static void P_BulletSlope(mobj_t *mo)
{
  ...
  fixed_t mlookslope; // Hakx 2/14/00

  // killough 8/2/98: make autoaiming prefer enemies
  int mask = demo_version < 203 ? 0 : MF_FRIEND;

  mlookslope = FixedMul(mo->mlookangle, -40960);
  ...
  do
	...
	bulletslope = P_AimLineAttack(mo, an -= 2<<26, 16*64*FRACUNIT, mask);
      if (!linetarget)  // Hakx 2/14/00
        {
	  an = mo->angle;
          bulletslope = mlookslope;
        }
    }
  while (mask && (mask=0, !linetarget));  // killough 8/2/98
}
...
void A_FireMancubus(player_t *player, pspdef_t *psp) // Hakx 1/28/99
{
  if (!infinite_ammo)
     player->ammo[weaponinfo[player->readyweapon].ammo]--;
  P_SpawnPlayerMissile(player->mo, MT_FATSHOT);
}

void A_FireBaron(player_t *player, pspdef_t *psp)
{
  if (!infinite_ammo)
     player->ammo[weaponinfo[player->readyweapon].ammo]--;
  P_SpawnPlayerMissile(player->mo, MT_BRUISERSHOT);
}

void A_FireArachnotron(player_t *player, pspdef_t *psp)
{
  if (!infinite_ammo)
     player->ammo[weaponinfo[player->readyweapon].ammo]--;
  P_SpawnPlayerMissile(player->mo, MT_ARACHPLAZ);
}

void A_FireCaco(player_t *player, pspdef_t *psp)
{
  if (!infinite_ammo)
     player->ammo[weaponinfo[player->readyweapon].ammo]--;
  P_SpawnPlayerMissile(player->mo, MT_HEADSHOT);
}

void A_FireImp(player_t *player, pspdef_t *psp)
{
  if (!infinite_ammo)
    player->ammo[weaponinfo[player->readyweapon].ammo]--;
  P_SpawnPlayerMissile(player->mo, MT_TROOPSHOT);
}

// End Len's Codepointers

void A_FireBetaPlasma1(player_t *player, pspdef_t *psp)
{
  if (!infinite_ammo)
    player->ammo[weaponinfo[player->readyweapon].ammo]--;
...
void A_FireBetaPlasma2(player_t *player, pspdef_t *psp)
{
  if (!infinite_ammo)
    player->ammo[weaponinfo[player->readyweapon].ammo]--;
...
void A_FireRevenant(player_t *player, pspdef_t *psp)
{
  player->ammo[weaponinfo[player->readyweapon].ammo]--;
  P_SpawnPlayerMissile(player->mo, MT_TRACER);
}
