...
void P_CrossSpecialLine(line_t *line, int side, mobj_t *thing)
{
  ...
  if (!thing->player)
    switch(thing->type)    // Things that should NOT trigger specials...
      {
      case MT_TRACER:      // Fix v1.1 Lift Bug, Hakx 2/8/99
      case MT_FATSHOT:
      case MT_ARACHPLAZ:
...
void P_SpawnSpecials (void)
{
  ...
  for (i=0; i<numlines; i++)
    switch (lines[i].special)
      ...
      case 301:
        grav = ((sides[*lines[i].sidenum].sector->ceilingheight)-
                (sides[*lines[i].sidenum].sector->floorheight));
        for (s = -1; (s = P_FindSectorFromLineTag(lines+i,s)) >= 0;)
          sectors[s].sec_gravity = grav;
        break;
