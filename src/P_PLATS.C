...
void T_PlatRaise(plat_t* plat)
{
  ...
  switch(plat->status)
  {
    case up: // plat moving up
      ...
      {
        if (!(leveltime&7) && !silentmove(plat->sector)) // [SM Code] Hakx 8/6/1999 -thanks to Fraggle
      ...
      if (res == crushed && (!plat->crush))
      {
        ...
        plat->status = down;
        if (!silentmove(plat->sector)) // [SM Code] Hakx 8/6/1999 -thanks to Fraggle
  ...
  while ((secnum = P_FindSectorFromLineTag(line,secnum)) >= 0)
  {
    ...
    switch(type)
    {
      case raiseToNearestAndChange:
        ...
        if (!silentmove(sec)) // [SM Code] Hakx 8/6/1999 -thanks to Fraggle
