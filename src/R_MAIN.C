...
/*  for (i=0 ; i<viewheight ; i++)
  ...
  // planes
  for (i=0 ; i<(viewheight*2) ; i++) // [Mlook Code]
     {
      fixed_t dy = ((i-viewheight)<<FRACBITS)+FRACUNIT/2;
      dy = abs(dy);
      origyslope[i] = FixedDiv ( (viewwidth)/2*FRACUNIT, dy);
...
{               
  ...
  // [Mlook Code] Hakx 9/1/1999
  dy = FixedMul (viewheight << (FRACBITS/2), player->mo->mlookangle) >> 9;
  yslope = origyslope + (viewheight >> 1) + dy;
  centery = (viewheight >> 1) - dy;
  centeryfrac = centery << FRACBITS;

/*  // mlook            Hakx 2/21/99
  centery=viewheight/2+(mlookangle>>16);
  centeryfrac=((viewheight/2)<<16)+mlookangle;
  yslope=origyslope+(viewheight/2)-(mlookangle>>16);
*/
}
