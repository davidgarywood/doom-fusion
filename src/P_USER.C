...
// Hakx 8/23/1999
// Extended Buttons support:

  if (cmd->buttons2 & BT2_JUMP) // [Jump Code]
    {
     if (onground && !player->jumpdelay)
       {
...
/*  if (cmd->buttons2 & BT2_JUMP) // [Jump Code]
    {
     if (!player->jumpdelay || (player->jumpdelay <= 0))
       {
        if ((player->mo->z + player->mo->height < player->mo->ceilingz) &&
            (player->mo->z > player->mo->subsector->sector->floorheight))
          {
             player->mo->momz += jumpheight * FRACUNIT;

              // Hakx 10/19/1999 (Taking account of sector's gravity etc into jumpdelay)
              player->jumpdelay = ((1*TICRATE)); // /(sv_gravity/10));
              if (player->mo->subsector->sector->sec_gravity)
                {
                 player->jumpdelay = ((player->jumpdelay)/
                                     ((player->mo->subsector->sector->sec_gravity)/10));
                }
          }
       }
    } */
  ...
  if (mlookon)  // [Mlook Code] Hakx 9/1/1999
    {
     // player->mlookangle +=cmd->mlookangle;
     int angle = cmd->mlookangle;
     if (angle)
       {
	if (angle == -32768)
	  {
           // center view
           player->mo->mlookangle = 0;
          }
        else if (angle > 0)
            {
             // look down
             player->mo->mlookangle += angle;
             if (player->mo->mlookangle > FRACUNIT)
               player->mo->mlookangle = FRACUNIT;
            }
            else
                {
		 // look up
