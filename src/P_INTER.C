...
int obituaries; // Extern'ed in doomstat.h, Hakx 8/9/1999

void P_Obituary(mobj_t *target, mobj_t *source, mobj_t *inflictor)
{
/* int weapontype;
 int enemytype;
 static char* player_kill_msg;

 if (!target->player || !obituaries) // Not a player getting killed
   return;                           // or obituaries not on

 if (!source && !inflictor) // Environment kill
   {
    dprintf("%s died due to hostile environment", target->player->name);
   } else
       if (!source->player) // Monster Kill
         {
          dprintf("%s died to monster", target->player->name);
         } else
          if (source->player) // A player-2-player kill
            {
             if (source == inflictor) // Bullet/Punch/Chainsaw kill
               {
                weapontype = source->player->readyweapon;
               } else
                 {
                  switch (inflictor->type)
                        {
                         ...
                         case MT_PLASMA :
                              ...
                              break;
                         case MT_PLASMA1 :
                              weapontype = wp_betaplasma1;
                              break;
                         case MT_PLASMA2 :
                              weapontype = wp_betaplasma2;
                              break;
                        }
                 }

             if (source == target) // Self kill
               {
                switch (weapontype)
                      {
                       case wp_fist :
                            player_kill_msg = s_SELF_OBT_FIST;
                            break;
                       case wp_pistol :
                            player_kill_msg = s_SELF_OBT_PISTOL;
                            break;
                       case wp_shotgun :
                            player_kill_msg = s_SELF_OBT_SHOTGUN;
                            break;
                       case wp_chaingun :
                            player_kill_msg = s_SELF_OBT_CHAINGUN;
                            break;
                       case wp_missile :
                            player_kill_msg = s_SELF_OBT_RL;
                            break;
                       case wp_plasma :
                            player_kill_msg = s_SELF_OBT_PLASMA;
                            break;
                       case wp_bfg :
                            player_kill_msg = s_SELF_OBT_BFG;
                            break;
                       case wp_chainsaw :
                            player_kill_msg = s_SELF_OBT_CHAINSAW;
                            break;
                       case wp_supershotgun :
                            player_kill_msg = s_SELF_OBT_SUPERSHOTGUN;
                            break;
                       case wp_betaplasma1 :
                            player_kill_msg = s_SELF_OBT_BETAPLASMA1;
                            break;
                       case wp_betaplasma2 :
                            player_kill_msg = s_SELF_OBT_BETAPLASMA2;
                            break;
                       dprintf(player_kill_msg, target->player->name);
                      }
                else
                {
                 switch (weapontype)
                      {
                       case wp_fist :
                            player_kill_msg = s_OBT_FIST;
                            break;
                       case wp_pistol :
                            player_kill_msg = s_OBT_PISTOL;
                            break;
                       case wp_shotgun :
                            player_kill_msg = s_OBT_SHOTGUN;
                            break;
                       case wp_chaingun :
                            player_kill_msg = s_OBT_CHAINGUN;
                            break;
                       case wp_missile :
                            player_kill_msg = s_OBT_RL;
                            break;
                       case wp_plasma :
                            player_kill_msg = s_OBT_PLASMA;
                            break;
                       case wp_bfg :
                            player_kill_msg = s_OBT_BFG;
                            break;
                       case wp_chainsaw :
                            player_kill_msg = s_OBT_CHAINSAW;
                            break;
                       case wp_supershotgun :
                            player_kill_msg = s_OBT_SUPERSHOTGUN;
                            break;
                       case wp_betaplasma1 :
                            player_kill_msg = s_OBT_BETAPLASMA1;
                            break;
                       case wp_betaplasma2 :
                            player_kill_msg = s_OBT_BETAPLASMA2;
                            break;
                       }
                       dprintf(player_kill_msg, source->player->name, target->player->name);
                      }
               }
            } else // Unknown death
                  {
                   dprintf("death not defined");
                  }
*/}
...
void P_DamageMobj(mobj_t *target,mobj_t *inflictor, mobj_t *source, int damage)
{
  ...
  if ((target->health -= damage) <= 0)
    {
      if (target->player)
        P_Obituary(target, source, inflictor);
