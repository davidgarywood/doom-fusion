...
static boolean P_LookForPlayers(mobj_t *actor, boolean allaround)
{
  ...
  for (;; actor->lastlook = (actor->lastlook+1)&(MAXPLAYERS-1))
	...
	continue;
      ...
      // Hakx 7/19/1999 changed from 60 to a random number
      // to decrease predictablity
	actor->threshold = P_Random(pr_threshold);
...
void A_Massacre()
{
  ...
  //
  // Hakx 7/19/1999: Copied this over as a codepointer, for functionality
  //
...
// Hakx added on: 1/27/99 and 1/28/99
// Len's Codepointers:

void A_DistanceDamage(mobj_t *actor)
{
  if (!actor->target)
    return;
...
void A_SplitAttack(mobj_t *actor)
{
  if (!actor->target)
    return;
  if (P_CheckMeleeRange(actor))
...
void A_ProjectileFire(mobj_t *actor)
{
  if (!actor->target)
    return;
  A_FaceTarget(actor);
  P_SpawnMissile(actor, actor->target, (actor->state->misc1-1));
}


void A_DetonateMisc(mobj_t *mo)
{
  P_RadiusAttack(mo, mo->target, mo->state->misc1);
}

void A_ProjectileFire2(mobj_t *actor)
{
  if (!actor->target)
    return;
  P_SpawnMissile(actor, actor->target, (actor->state->misc1-1));
...
void A_EnemyFireBFG(mobj_t *actor)  // Hakx 1/31/99
{
  if (!actor->target)
    return;
  A_FaceTarget(actor);
  P_SpawnMissile(actor, actor->target, MT_BFG);  // Launch BFG
}

void A_EnemyFirePlasma(mobj_t *actor) // Hakx 1/31/99
{
  mobj_t* missile;
  if (!actor->target)
    return;
  A_FaceTarget(actor);
  missile = P_SpawnMissile(actor, actor->target, MT_PLASMA);  // Launch Plasma
  P_SetTarget(&missile->tracer, actor->target);  // Hakx 2/27/99
}

void A_EnemyFireBetaPlasma1(mobj_t *actor) // Hakx 1/31/99
{
  if (!actor->target)
    return;
  A_FaceTarget(actor);
  P_SpawnMissile(actor, actor->target, MT_PLASMA1);  // Launch Beta Plasma1
}

void A_EnemyFireBetaPlasma2(mobj_t *actor) // Hakx 1/31/99
{
  if (!actor->target)
    return;
  A_FaceTarget(actor);
  P_SpawnMissile(actor, actor->target, MT_PLASMA2);  // Launch Beta Plasma1
}

void A_EnemyFireRocket(mobj_t *actor) // Hakx 1/31/99
{
  if (!actor->target)
    return;
  A_FaceTarget(actor);
  P_SpawnMissile(actor, actor->target, MT_ROCKET);  // Launch Rocket
}

// Hakx 7/19/1999 [Fire FX]
void A_Burn(mobj_t *thingy)
{
  P_RadiusAttack(thingy, thingy->target, 20);
}

// Flag Changers, suggested by Len Pitre Hakx 7/19/99

void A_MFC_Shadow(mobj_t *actor) // Changes Flag to Opposite of what is currently is
{
...
void A_MFC_ShadowOn(mobj_t *actor) // Makes sure flag is on
{
 actor->flags |= MF_SHADOW;
}

void A_MFC_ShadowOff(mobj_t *actor) // Makes sure flag is off
{
 actor->flags &= ~MF_SHADOW;
}

void A_MFC_Float(mobj_t *actor) // Changes Flag to Opposite of what is currently is
{
...
void A_MFC_FloatOn(mobj_t *actor) // Makes sure flag is on
{
 actor->flags |= MF_FLOAT;
}

void A_MFC_FloatOff(mobj_t *actor) // Makes sure flag is off
{
 actor->flags &= ~MF_FLOAT;
}

void A_MFC_Touchy(mobj_t *actor) // Changes Flag to Opposite of what is currently is
{
 if (actor->flags & MF_TOUCHY)
 {
  actor->flags &= ~MF_TOUCHY;
 }
 else
 {
  actor->flags |= MF_TOUCHY;
...
void A_MFC_TouchyOn(mobj_t *actor) // Makes sure flag is on
{
 actor->flags |= MF_TOUCHY;
}

void A_MFC_TouchyOff(mobj_t *actor) // Makes sure flag is off
{
 actor->flags &= ~MF_TOUCHY;
}

void A_MFC_Friend(mobj_t *actor) // Changes Flag to Opposite of what is currently is
{
 if (actor->flags & MF_FRIEND)
 {
  actor->flags &= ~MF_FRIEND;
...
void A_MFC_FriendOn(mobj_t *actor) // Makes sure flag is on
{
 actor->flags |= MF_FRIEND;
}

void A_MFC_FriendOff(mobj_t *actor) // Makes sure flag is off
{
 actor->flags &= ~MF_FRIEND;
}

void A_MFC_Translucent(mobj_t *actor) // Changes Flag to Opposite of what is currently is
{
...
void A_MFC_TranslucentOn(mobj_t *actor) // Makes sure flag is on
{
 actor->flags |= MF_TRANSLUCENT;
}

void A_MFC_TranslucentOff(mobj_t *actor) // Makes sure flag is off
{
 actor->flags &= ~MF_TRANSLUCENT;
}

void A_MFC_Bounces(mobj_t *actor) // Changes Flag to Opposite of what is currently is
{
 if (actor->flags & MF_BOUNCES)
 {
  actor->flags &= ~MF_BOUNCES;
 }
 else
 {
  actor->flags |= MF_BOUNCES;
...
void A_MFC_BouncesOn(mobj_t *actor) // Makes sure flag is on
{
 actor->flags |= MF_BOUNCES;
}

void A_MFC_BouncesOff(mobj_t *actor) // Makes sure flag is off
{
 actor->flags &= ~MF_BOUNCES;
}

void A_MFC_Noblood(mobj_t *actor) // Changes Flag to Opposite of what is currently is
{
...
void A_MFC_NobloodOn(mobj_t *actor) // Makes sure flag is on
{
 actor->flags |= MF_NOBLOOD;
}

void A_MFC_NobloodOff(mobj_t *actor) // Makes sure flag is off
{
 actor->flags &= ~MF_NOBLOOD;
}
...
// Hakx 8/21/1999
// Forward moving codepointer
//

void A_MoveForward(mobj_t *actor)
{
  angle_t an;
  fixed_t misc1 = actor->state->misc1 ? actor->state->misc1 : 4;

  an = actor->angle;

  an >>= ANGLETOFINESHIFT;
  actor->momx += FixedMul ((misc1*FRACUNIT), finecosine[an]);
  actor->momy += FixedMul ((misc1*FRACUNIT), finesine[an]);
}

// Hakx 8/21/1999
// Backward moving codepointer
//

void A_MoveBackward(mobj_t *actor)
{
  angle_t an;
  fixed_t misc1 = actor->state->misc1 ? actor->state->misc1 : 4;
...
// Hakx 8/21/1999
// Upwards moving codepointer
//

void A_MoveUp(mobj_t *actor)
{
  fixed_t misc1 = actor->state->misc1 ? actor->state->misc1 : 4;

  actor->momz += ((misc1*FRACUNIT));
}
...
void A_ShooterPistol(mobj_t* actor)    // Shoots Pistol
{
  int t = P_Random(pr_posattack);
  int angle;
  int damage = ((P_Random(pr_posattack)%5)+1)*3;
  angle = actor->angle + ((t - P_Random(pr_posattack))<<20);
  S_StartSound(actor, sfx_pistol);
  P_LineAttack(actor, angle, MISSILERANGE, 0, damage);
}

void A_ShooterShotgun(mobj_t* actor)   // Shoots Shotgun
{
 int i;
 S_StartSound(actor, sfx_shotgn);
 for (i=0; i<3; i++)
 {
  ...
  int angle = actor->angle + ((t - P_Random(pr_shotgun))<<20);
  int damage = ((P_Random(pr_shotgun)%5)+1)*3;
  P_LineAttack(actor, angle, MISSILERANGE, 0, damage);
 }
}

void A_ShooterDoubleShotgun(mobj_t* actor)   // Shoots Double Shotgun
{
 int i;
...
void A_ShooterRocket(mobj_t* actor)
{
 P_SpawnShooterMissile(actor, MT_ROCKET);  // Shoots out MT_ROCKET
}

void A_ShooterPlasma(mobj_t* actor)
{
 P_SpawnShooterMissile(actor, MT_PLASMA);  // Shoots out MT_PLASMA
}

void A_ShooterBFG(mobj_t* actor)
{
 P_SpawnShooterMissile(actor, MT_BFG);  // Shoots out MT_BFG
}

void A_ShooterBetaPlasma1(mobj_t* actor)
{
 P_SpawnShooterMissile(actor, MT_PLASMA1);  // Shoots out MT_PLASMA1
}

void A_ShooterBetaPlasma2(mobj_t* actor)
{
 P_SpawnShooterMissile(actor, MT_PLASMA2);  // Shoots out MT_PLASMA2
}
