...
int     key_jump; // Hakx 8/23/1999 [Jump Code]
int     key_right;
int     key_left;
int     key_up;
int     key_down;
...
void G_BuildTiccmd(ticcmd_t* cmd)
{
  ...
  // Hakx 8/10/1999 -Set player's name etc:
  if ((gamestate == GS_LEVEL) && (!demoplayback)) // If in game, and no demo playing
    {
     players[consoleplayer].name = cl_name;
    }
...
/*  forward += mousey;
  ...
  if (!mlookon)
    forward += mousey;
      ...
      {
       if (!invertmouse)
        cmd->mlookangle+=ml_mousey*viewheight*((mlookspeed)>>4);
       else
        cmd->mlookangle-=ml_mousey*viewheight*((mlookspeed)>>4);
      }
  if (strafe)
...
boolean G_Responder(event_t* ev)
{
  ...
  switch (ev->type)
    ...
    case ev_mouse:
      ...
      ml_mousex = (ev->data2*4)/10; // Hakx 2/13/00
      ml_mousey = (ev->data3*4)/10;
...
static void G_ReadDemoTiccmd(ticcmd_t *cmd)
{
  if (*demo_p == DEMOMARKER)
  ...
  else
    {
      ...
      if (demo_version > 203)
        cmd->buttons2 = (unsigned char)*demo_p++; // Hakx 8/23/1999
                                                  // extended buttons support
                                                  // for jumping and swimming in water
...
void G_BeginRecording(void)
{
  ...
  *demo_p++ = VERSION;
  ...
  // signature
  *demo_p++ = 0x1d;
  *demo_p++ = 'F';   // Changed to 'FUS' Hakx 10/19/99
  *demo_p++ = 'U';
  *demo_p++ = 'S';
  *demo_p++ = 0xe6;
  *demo_p++ = '\0';
