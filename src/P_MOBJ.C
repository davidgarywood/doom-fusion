...
#include "p_spec.h" // Hakx 8/2/1999
...
static void P_ZMovement (mobj_t* mo)
{
  ...
  local_grav = ((sv_gravity*GRAVITY)/10);

  if (mo->subsector->sector->sec_gravity)
    {
     local_grav = ((local_grav*(mo->subsector->sector->sec_gravity))/10);
    }
      ...
      if (mo->z <= mo->floorz)                  // bounce off floors
	{
	  ...
	  if (mo->momz < 0)
	      ...
	      if (!(mo->flags & MF_NOGRAVITY))  // bounce back with decay
		{
		  ...
		  if (abs(mo->momz) <= mo->info->mass*((local_grav)*4/256))
		    mo->momz = 0;
		}
	...
	if (mo->z >= mo->ceilingz - mo->height)   // bounce off ceilings
	...
	else
	  {
	    if (!(mo->flags & MF_NOGRAVITY))      // free-fall under gravity
	      mo->momz -= mo->info->mass*((local_grav)/256);
	    if (mo->flags & MF_FLOAT && sentient(mo))
	      goto floater;
	    return;
	  }
...
void P_SpawnPlayerMissile(mobj_t* source,mobjtype_t type)
{
  ...
  mlookslope = FixedMul (source->mlookangle, -40960);
...
// Hakx 10/7/1999 Added P_SpawnShooterMissile
// This code is almost a duplicate of P_SpawnMissile except that
// instead of getting it's angle from the comparable angle between the
// source and the target, it just shoots out with the same angle as the
// source. This is needed, as obviously Shooters need to shoot out straight
// in front, and as non-badguys they don't gain targets.

mobj_t* P_SpawnShooterMissile(mobj_t* source, mobjtype_t type)
{
  angle_t an;
  mobj_t *th = P_SpawnMobj (source->x,source->y,source->z + 4*8*FRACUNIT,type);
  ...
  th->z = source->z + (source->height>>1) + 8*FRACUNIT;
  P_CheckMissileSpawn(th);
  return th;
}
