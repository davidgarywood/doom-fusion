...
// Hakx 5/17/99 lets 1s lindefs with no texture be rendered as passable
int      single_side_pass;
...
static void R_AddLine (seg_t *line)
{
  ...
  // Hakx 3/29/99, if a single sided line has no texture, draw as a passable
  // this enables 'windows' to be created;
  // * the single-sided still blocks players/monsters and projectiles/bullets
  // * a 2-sided on the other side with the glass texture

  if (single_side_pass && (!backsector && curline->sidedef->midtexture == 0))
    {
     goto clippass;
     return;
    }
