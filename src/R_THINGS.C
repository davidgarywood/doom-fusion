...
void R_InitSpriteDefs(char **namelist)
{
  ...
  // 5/22/99 Hakx -CHANGED TO PU_CACHE just to see if any difference
...
// Removed Static -- Hakx 5/14/99
//
// Hakx 7/26/1999
//
// This is a BIG performance issue; when static'ed, Rez found that
// the 'herky jerky' lag build up that the DJGPP compiler seems to
// induce, that builds up over runtime (after about 40-60 mins of play
// -can be induced quicker with savegame load/reloading)
// Removal of this found that these were eliminated
//
      ...
      if (vis->mobjflags & MF_TRANSLUCENT && general_translucency) // phares
        {
          ...
          // [TT Code] -- Hakx 7/31/1999
          if (tt_table[vis->type].status != -1)
            {
             static int lump;
             static char* mapname;

             mapname = tt_table[vis->type].tt_mapname;
             lump = W_CheckNumForName(mapname);

             if (lump != -1) // If lump exists use it
               {
                tranmap = W_CacheLumpNum(lump, PU_STATIC);
               } else tt_table[vis->type].status = -1;
...
/*  if (thing->z > viewz + FixedDiv(centeryfrac, xscale) ||
  ...
  vis->type = thing->type; // Hakx 7/31/1999
...
int cl_drawweapon; // Hakx 2/14/00
...
void R_DrawPSprite (pspdef_t *psp)
{
  ...
  // Draw Weapon
  if (cl_drawweapon) // Hakx 2/14/00
    R_DrawVisSprite(vis, vis->x1, vis->x2);

  // Switch Mlook 'on' again
  dy = FixedMul (viewheight << (FRACBITS/2), viewplayer->mo->mlookangle) >> 9;
