...
{
	int pnum = (viewplayer->mo->subsector->sector - sectors) * numsectors;
...
void P_RunEffect (mobj_t *actor, int effects)
{
	...
	if ((effects & FX_ROCKET) && rockettrails) {
		// Rocket trail
	...
	if ((effects & FX_GRENADE) && (rockettrails)) {
		// Grenade trail
...
void P_DrawRailTrail (vec3_t start, vec3_t end)
{
	...
	if (length) {
		...
		{ // This player fired the railgun
                  S_StartSound(mo, sfx_None/*s_railgun*/);
		}
		else
		{
