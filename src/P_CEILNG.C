...
void T_MoveCeiling (ceiling_t* ceiling)
{
  ...
  switch(ceiling->direction)
  {
    ...
    case 1:
      ...
      if (!(leveltime&7) && !silentmove(ceiling->sector)) // [SM Code] Hakx 8/6/1999 -thanks to Fraggle (Simon Howard)
      {
        switch(ceiling->type)
        {
      ...
      if (res == pastdest)
      {
        switch(ceiling->type)
        {
          ...
          case silentCrushAndRaise:
            if (!silentmove(ceiling->sector)) // [SM Code] Hakx 8/6/1999 -thanks to Fraggle
